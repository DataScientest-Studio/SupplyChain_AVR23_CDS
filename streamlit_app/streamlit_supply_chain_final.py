...
def afficher_partie3():
    ...
    with st.expander("2-Evolution des notes",expanded=False):
        ...
        -	Sephora : a connu une chute de ses notes entre 2012 et 2019 mais semble, depuis, avoir de meilleures notes
        -	Nocibé : a de mauvaises notes depuis 2016 sans connaitre d'évolutions majeures \n
        Parmis les entreprises ayant de très bonnes notes :\n
         -	Marionnaud : connait une évolution croissante de ses notes depuis 2021\n
        La note moyenne de 2 en 2021 ne doit pas forcément être interprétée comme une mauvaise note. En effet, le scrapping ne couvre pas toute l'année 2022 et la note est ainsi biaisée.\n
    ...
    with st.expander("3-Service après vente",expanded=False):
        ...
        Sephora ne semble pas avoir de service clients.\n
    ...
    with st.expander("4-Commentaires clients",expanded=False):
        ...
        Nous avons donc représenté deux wordcloud pour chacune des entreprises : 
        ...
        st.write("""
        Contrairement aux avis négatifs, aucun point commun ne semble se dégager pour les commentaires positifs qui sont beaucoup plus spécifiques : 
...
def afficher_partie4():
   ...
   ## RANDOM FOREST SUR JEU COMPLET APRES SMOTE
    ...
    report = classification_report(y_train, y_pred_rfc_train)
    ...
    report2 = classification_report(y_test, y_pred_rfc_test)
    ...
    report3 = classification_report(y_train, y_pred_svc_train)
    ...
    report4 = classification_report(y_test, y_pred_svc_test)
    ...
    X_train_text = loaded_tokenizer.texts_to_sequences(X_train)
    X_test_text = loaded_tokenizer.texts_to_sequences(X_test)
    ...
    loss, accuracy = embedding_model.evaluate(train_text, y_train_cat)
    st.write("L accuracy sur notre jeu d entrainement est : ", accuracy)
    
    loss, accuracy = embedding_model.evaluate(test_text, y_test_cat)
    st.write("L accuracy sur notre jeu de test est : ", accuracy)

    # Utilisation de StringIO pour capturer le résumé du modèle
    import io
    import sys
    ...
    # Affichage du résumé du modèle dans l'application Streamlit
...
def afficher_partie5():
    ...
    def prediction_note(commentaire, model):
        if model == 'Random Forest':
            commentaire = vectorizer.transform([commentaire]).toarray()
            commentaire = np.squeeze(commentaire)
            ...
            max_len = 100  
            commentaire_text = loaded_tokenizer.texts_to_sequences([commentaire])
            commentaire_text = pad_sequences(commentaire_text, maxlen=max_len)[0]
            commentaire_text = np.expand_dims(commentaire_text, axis=0)
            proba_prediction = embedding_model.predict(commentaire_text)[0]
            predicted_note = round(np.argmax(proba_prediction) + 1)
    ...
    if st.button("Prédire"):
        if commentaire_input:
            predicted_note = prediction_note(commentaire_input, model)
